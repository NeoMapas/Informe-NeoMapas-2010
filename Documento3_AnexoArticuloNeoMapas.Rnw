...
\usepackage[natbib=true, citestyle=authoryear,bibstyle=authoryear, dashed=false, maxnames=2, firstinits=true]{biblatex}
\bibliography{/Users/jferrer/NeoMapas/lib/BibTEX/DocumentosNM,%
/Users/jferrer/NeoMapas/lib/BibTEX/paquetesR,%
/Users/jferrer/NeoMapas/lib/BibTEX/revlit}
...
\pretitle{\begin{center} 
 \par \large Supporting information for \emph{Filling the gaps in national biodiversity surveys: NeoMaps as a model for tropical regions} \\[0.2cm] \LARGE }
\posttitle{\par \\[0.3cm]
...
\postauthor{%\\
%Centro de Ecología y Centro de Estudios Botánicos y Agroforestales\\Instituto Venezolano de Investigaciones Científicas
\end{center}
\par}
...
\title{\textbf{S3 Brief summary of NeoMaps data}}
\author{José Rafael Ferrer-Paris, Jon Paul Rodríguez, Tatjana C. Good, Ada Y. Sánchez-Mercado, Kathryn M. Rodríguez-Clark, Gustavo A. Rodríguez and Angel Solís}

\begin{document}
\maketitle
%\bibliographystyle{/Users/jferrer/NeoMapas/lib/custom-bib/tesis}

%\section{Brief summary of NeoMaps data}
...
NeoMaps has \Sexpr{nrow(mi.info)} active transects, which has been sampled for at least one taxonomic group between 2001 and 2010. Table \ref{TAB:RSMINFO} give a brief summary on each transect, more details will be available in NeoMaps home page \url{http://www.neomaps.org}. Methods calibration was performed between 2001 and 2003 for birds (\emph{AV01} in table \ref{TAB:RSMINFO}), between 2003-2005 for butterflies (\emph{PP03}), and in 2005 for dung beetles (\emph{SC05}). In 2006 we completed the first national survey for butterflies (\emph{PP06}) and a pilot study of the dung beetle survey (\emph{SC06}), between 2009 and 2010 we completed the second national butterfly survey (\emph{PP09}), the first national dung beetle survey (\emph{SC09}) and the first national bird survey (\emph{AV10}).

\LTcapwidth=\textwidth

<<tabla general, results=tex,echo=false>>=
...
Between 2003 and 2010, NeoMaps has sampled butterflies in \Sexpr{luq(mrps.info$NM)} transects (see table \ref{TAB:MRPSINFO}). Total sampling effort was \Sexpr{round(sum(mrps.info$x,na.rm=T),1)} hours*collector, which resulted in \Sexpr{sum(mrps.info$jmp,na.rm=T)} individuals captured. Identification of specimens is still in progress, specially for transects collected in 2009 and 2010, and for families Hesperiidae and Lycaenidae.

For the present study we focused on \Sexpr{sum(mrps.NM)} individuals of the family Pieridae collected in \Sexpr{nrow(mrps.NM)} different transects sampled in the first national survey (2006). Specimens were identified by José R. Ferrer-Paris, \Sexpr{ncol(mrps.NM)} species were identified (see below).

Estimates of total species richness, mean abundances and diversity of Pieridae are presented in table \ref{TAB:MRPSEST}, estimates of biotic dissimilarity are given in table \ref{TAB:MRPSDIS} (see methods section in main document). Detailled data on species records and summaries for each transect will be available in NeoMaps web page (\url{http://www.neomaps.org}).
...
\begin{table}[H]
\caption{Estimates of biotic dissimilarity between pairs of butterfly samples based on \emph{Chao} index.}
\begin{center}
<<MTRZmrps,eval=true, echo=false, results=hide, fig=true, width=7, height=7>>=
...
\subsection*{Species of Pieridae detected by NeoMaps}

The following list is based in the samples included in this analysis. Taxonomic names and higher classification follow \citet{Lamas2004}. 
...
Between 2005 and 2010, NeoMaps has sampled dung beetles in \Sexpr{luq(scrb.rslt$NM)} transects. Total sampling effort was \Sexpr{round(sum(scrb.rslt$sfrz,na.rm=T),1)} hours*trap, which resulted in \Sexpr{sum(scrb.rslt$njmp,na.rm=T)} individuals captured. Identifications were made by Antonio D'Ascencao (ULA, Mérida) for the samples from 2005 and Angel Solís (INBio, Costa Rica) for all other samples. Samples from transects with codes \Sexpr{NMNs} from 2009 and 2010 have not been identified yet, and transects 41 and 15, have incomplete identifications. Transects with codes \Sexpr{NM2s} were sampled in two different years. 

For the present study we excluded transects sampled in 2005 during the methods calibration period, as well as the samples with incomplete identifications mentioned above, or insufficient sampling effort, and selected the most recent sample when more than one was available. This resulted in a data set with \Sexpr{nrow(scrb.NM)} different transects and \Sexpr{sum(scrb.NM)} individuals from \Sexpr{ncol(scrb.NM)} species/morphospecies (see below). 

Estimates of total species richness, mean abundances and diversity are presented in table \ref{TAB:SCRBEST}, estimates of biotic dissimilarity are given in table \ref{TAB:SCRBDIS} (see methods section in main document). Detailled data on species records and summaries for each transect will be available in NeoMaps web page (\url{http://www.neomaps.org}).
...
\begin{table}[H]
\begin{center}
\caption{Estimates of biotic dissimilarity between pairs of dung beetle transects based on \emph{Chao} index.}

<<MTRZscrb,eval=true, echo=false, results=hide, fig=true, width=7, height=7>>=
...
\subsection*{Species and morphotypes of Scarabaeinae detected by NeoMaps}

The following list is based in the samples included in this analysis. Taxonomic names follow \citet{scarabnet} and \citet{Vaz-de-Mello:2007kx}, higher classification according to \citet{Bouchard2011}. Unnamed morphotypes are coded for reference in NeoMaps database.
...
for (j in unique(NM.scarab$trb)) {
	...
	for (k in strb) {
		...
		for (i in seq(along=NM.scarab$trb)[NM.scarab$trb %in% j & NM.scarab$strb %in% k]) {
			cat(paste("\n\\item", sub("<i>", "\\\\textit{",sub("</i>","}", sub("<A>",sub("&","\\&",NM.scarab$autor[i]), sub("<F>",NM.scarab$fecha[i], sub("<E>",NM.scarab$epiteto[i], sub("<G>",NM.scarab$genero[i],NM.scarab$sintax[i]))))))))
		}
		cat(paste("\n\\end{enumerate}\n"))
	}
	cat(sprintf("\\end{itemize}"))
...
t0 <- t0[order(t0$NM),c("NM","nreg.m1","nind.m1", "nspp.m1", "nreg.m2", "nspp.m2", "nspp")]


sd.scrb <- apply(log1p(aves.NM),1,sd)
abnd.scrb <- apply(log1p(aves.NM),1,mean)
H0.scrb <- diversity(aves.NM, index = "shannon")
H1.scrb <- renyi(aves.NM,scales=1)
ACE.scrb <- estimateR(aves.NM)["S.ACE",]
## realmente, según el código de estimateR es standard deviation
se.scrb <- estimateR(aves.NM)["se.ACE",]


aves.rslt <- data.frame(NM=trans.info$NM[match(rownames(aves.NM),trans.info$Nombre)], log.abnd=abnd.scrb, sd.log.abnd=sd.scrb, H0=H0.scrb, ACE=ACE.scrb, se.ACE=se.scrb)

t1 <- merge(t0,aves.rslt,by="NM")


@

Between 2001 and 2003 four transects (\Sexpr{paste(tt$NM,collapse=", ")}) were sampled as part of a methodological essay. Total sampling effort was \Sexpr{sum(tt$sfrz)} hours*person of observations. In 2010 \Sexpr{nrow(t0)} transects were sampled with two different methods. The first day 50 three-minute stops were sampled and individuals counted. The second day cummulative species list were made for 10 nine-minute stops, and no data on individual counts were recorded. See table \ref{TAB:AVESRSM} for summaries of unique records, individuals counted and species detected, and estimates of abundance, richness and diversity based on first day counts. Estimates of biotic dissimilarity between transects are presented in table \ref{TAB:AVESDIS}.

<<echo=false,results=tex>>=
 print(xtable(t1, caption="Transects included in NeoMaps' Bird survey from 2010. NM: NeoMaps transect code (see table \\ref{TAB:RSMINFO} for details), $C$: number of unique records, $N$: total number of individuals counted (first day count), $S_{obs}$: number of species detected, $S_{total}$: total number of species detected in both days. Summary results based on first day counts: estimated mean and standard deviation of bird species abundance in log-scale  ($\\log N (\\mu \\pm \\sigma)$), Shannon-Weaver diversity index, ($H$), and abundance coverage estimator of bird species richness (ACE) for NeoMaps transects.", label="TAB:AVESRSM", align="|l|c|ccc|cc|c||rlcrl|", digits=c(0,0,0,0,0,0,0,0,3,3,3,3,3)), caption.placement="top", include.rownames=F, include.colnames=F, add.to.row=list(pos=list(-1), command=c("
\\multicolumn{1}{c}{} & \\multicolumn{3}{c}{First day count} &  \\multicolumn{2}{c}{Second day list} & \\multicolumn{1}{c}{} & \\multicolumn{5}{c}{Estimates from first day count}\\\\\\hline\n
 NM &  $C$ & $N$ & $S_{obs}$ &  $C$ & $S_{obs}$& $S_{total}$ & \\multicolumn{2}{|c}{$\\log N (\\mu \\pm \\sigma)$} & $H$ & \\multicolumn{2}{c|}{ACE $\\pm$ s.e.}\\\\")), table.placement="H", hline.after=c(-1,nrow(t0)), size="normalsize")
...
\begin{table}[H]
\begin{center}
\caption{Estimates of biotic dissimilarity between pairs of bird transects based on \emph{Chao} index.}
\label{TAB:AVESDIS}
<<MTRZaves,eval=true, echo=false, results=hide, fig=true, width=7, height=7>>=
...
%\section{Literature reviewed as alternative source of data}
...
@
%\subsection{Pieridae butterflies}

%We found literature records for \Sexpr{nrow(spps)} species in \Sexpr{length(unique(mrps.cneb$cdg_ref[mrps.cneb$validacion %in% c("exacto", "ortografico", "soundex epiteto",  "soundex genero", "manual") & mrps.cneb$adm0 %in% "VEN" & grepl("^097-",mrps.cneb$cdg)]))} references. 

%\begin{refsection}

<<eval=false,echo=false,results=tex>>=
...
@
%\end{refsection}
%\printbibliography[section=1, title=Published sources of distribution data for Pieridae butterflies, heading=subbibliography] 

%\subsection{Scarabaeinae dung beetles}

%Taxonomic names follow \citet{scarabnet}, \citet{Vaz-de-Mello:2007kx}, higher classification according to \citet{Bouchard2011}.
...
%We found literature records for \Sexpr{nrow(spps)} species in \Sexpr{length(unique(scrb.cneb$cdg_ref[scrb.cneb$validacion %in% c("heredada","manual","vld-auto :: concordancia exacta") & scrb.cneb$adm0 %in% "VEN"]))} references. 
...
@
%\end{refsection}
%\printbibliography[section=2, title=Published sources of distribution data for Scarabaeinae, heading=subbibliography, maxnames=20] 
